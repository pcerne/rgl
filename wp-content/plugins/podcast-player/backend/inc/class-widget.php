			...
			array(
				...
				'info'    => array(
					...
					'items' => array(
						...
						'pp_apple_sub'      => array(
							'setting' => 'pp_apple_sub',
							'label'   => esc_html__( 'Apple Podcast Subscription link.', 'podcast-player' ),
							'type'    => 'url',
						),
						'pp_google_sub'     => array(
							'setting' => 'pp_google_sub',
							'label'   => esc_html__( 'Google Podcast Subscription link.', 'podcast-player' ),
							'type'    => 'url',
						),
						'pp_spotify_sub'    => array(
							'setting' => 'pp_spotify_sub',
							'label'   => esc_html__( 'Spotify Podcast Subscription link.', 'podcast-player' ),
							'type'    => 'url',
						),
						'podcast_menu'      => array(
							'setting' => 'podcast_menu',
							'label'   => esc_html__( 'Custom menu for more subscription links', 'podcast-player' ),
							'type'    => 'select',
							'choices' => $menu_arr,
						),
						'number'            => array(
						...
						'pp_excerpt_length' => array(
							...
							'hide_callback' => function() use ( $widget, $settings ) {
								return ! Validation_Fn::is_style_support( $settings['pp_display_style'], 'excerpt' );
							},
						),
				...
				'show'    => array(
					...
					'items'       => array(
						'pp_header_default'   => array(
							...
							'type'          => 'checkbox',
							'hide_callback' => function() use ( $widget, $settings ) {
								return $widget->is_option_not_equal( 'pp_display_style', '', $settings ) && $widget->is_option_not_equal( 'pp_display_style', 'legacy', $settings );
							},
						),
	...
	public function form( $instance ) {
		...
		foreach ( $options as $option => $args ) {
			...
			if ( 'default' === $option ) {
				...
				$section        .= sprintf( '<div class="pp-settings-content pp-%1$s-content">%2$s</div>', $option, $markup );
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( $this->is_premium && isset( $instance['pp_fetch_method'] ) && 'feed' !== $instance['pp_fetch_method'] ) {
			...
			if ( isset( $old_instance['feed_url'] ) && ( $new_instance['feed_url'] === $old_instance['feed_url'] ) ) {
				...
				$validurl = Validation_Fn::is_valid_url( $new_instance['feed_url'] );
				if ( ! $validurl ) {
					$error = esc_html__( 'Feed URL is not valid', 'podcast-player' );
				}

				$feedurl = $new_instance['feed_url'];
				// Check if a valid url has been provided.
				if ( false !== strpos( $feedurl, 'http://' ) || false !== strpos( $feedurl, 'https://' ) ) {
					$feedurl = esc_url_raw( wp_strip_all_tags( $feedurl ) );
				} else {
					if ( is_int( $validurl ) && ! in_array( $validurl, array( 1, 2 ), true ) ) {
